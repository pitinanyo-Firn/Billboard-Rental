...
function route_(action, p) {
  try {
    switch (action) {
      case 'ping':             return { ok: true, service: 'Billboard Rental Hub API', version: CFG.VERSION };
      ...
      case 'getRental':        return apiRental_(p.token, p.id);
      ...
      case 'deleteMember':     return apiDeleteMember_(p.token, p);
