...
/** ตรวจ token + ต้องเป็น admin — คืน profile หรือ null */
function authAdmin_(token) {
  const me = auth_(token);
  return me.role === 'admin' ? me : null;
}

function forbidden_(what) {
  return { ok: false, code: 'FORBIDDEN', message: 'เฉพาะ admin เท่านั้นที่' + what + 'ได้' };
}
